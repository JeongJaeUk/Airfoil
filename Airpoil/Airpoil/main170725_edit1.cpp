...
void DescribeMesh() { // 재 : Cell => CellShape 2017-01-12 //0208
	...
	for (unsigned int k = 0; k<faceVector.size(); k++) {
		...
		for (int i = 0; i<FACount[di][lowCount]; i++) {
			...
			/*if (sfi >= cellVector.size() + minimalCellVector_tri.size()) {
				sfi = 0;
			}*/
			if (e != NULL) efi = e->getID();//0208
			/*if (efi >= cellVector.size() + minimalCellVector_tri.size()) {
				efi = 0;
			}*/
...
int main() {
	...
	fp = fopen("Test_07_31_uk_layer1_depth1_hexa_2up_error_draw.msh", "w");
