	...
	for (unsigned int k = 0; k<faceVector.size(); k++) {
		...
		for (int i = 0; i<FACount[di][lowCount]; i++) {
			...
			if (e != NULL) efi = e->getID();//0208
											/* 욱 : over발생시 0으로
											if (en > initRectangle->getNodeCount()) {
											en = 0;
											}
											if (sn > initRectangle->getNodeCount()) {
											sn = 0;
											}
											*/
			if (sfi > 255 || efi > 255) {
...
void Rectangle::splitCell(double x1, double y1, double x2, double y2, int layer, int splitDepth, int depth) {
	// 재 : Cell split 함수 입니다. 점 두개와 layer, splitDepth, depth를 입력받습니다. 2017-02-20
		...
		if (norf_f == 1) { //face이면
			if (!face_f->isParent()) wallsid = cutface(fnode, face_f, 2);// child가 없으면 쪼개고 중간점을 wall의 snode
				...
				wallsid = face_f->centerNode->id; //쪼개져있으면 centernode =  wall의 snode
												  //추가 2017_03_15 재
				face_f->centerNode->inout = -2; // 도형위의 점v
			}
			c->LFace->eCell = NULL;
		}
	...
	if (wopoint == 0) {//LU
		...
		if (norf_s == 1) {
			if (!c->UFace->isParent()) { //child 가없다면 쪼갠다.
				...
				c->UFace->setCenterNode(UPCenter);
				UpFace = ((Cell*)c->UFace->sCell)->DFace->Child1; // 욱 : 170724 Child1 = > Child2
			}
			else {
		...
		if (norf_f == 1) {
			if (!c->DFace->isParent()) { //child 가없다면 쪼갠다.
				...
				c->DFace->setCenterNode(DownCenter);
				DownFace = ((Cell*)c->DFace->eCell)->UFace->Child1; // 욱 : 170724 Child1 = > Child2
			}
			else {
		...
		if (norf_s == 1) {
			if (!c->LFace->isParent()) { //child 가없다면 쪼갠다.
				...
				c->LFace->setCenterNode(LeftCenter);
				LeftFace = ((Cell*)c->LFace->sCell)->RFace->Child1; // 욱 : 170724 Child1 = > Child2
			}
			else {
				LeftCenter = c->LFace->centerNode;
		...
		if (norf_f == 1) {
			if (!c->RFace->isParent()) { //child 가없다면 쪼갠다.
				...
				c->RFace->setCenterNode(RightCenter);
				RightFace = ((Cell*)c->RFace->eCell)->LFace->Child1; // 욱 : 170724 Child1 = > Child2
			}
			else {
				RightCenter = c->RFace->centerNode;
...
void tailcut_tri(Cell* c, pdd inode, pdd cnode, bool updown, int type) {// 진 : tail 설정  2017-03-07
	...
	if (type == 3) {
		c->LUCell = new Cell();	((Cell*)c->LUCell)->setDepth(c->depth + 1);
		cellVector.push_back(((Cell*)c->LUCell));
	}
	if (type == 2) {
		c->LDCell = new Cell();	((Cell*)c->LDCell)->setDepth(c->depth + 1);
		cellVector.push_back(((Cell*)c->LDCell));
	}
	if (type != 3) {
		c->RDCell = new Cell();	((Cell*)c->RDCell)->setDepth(c->depth + 1);
		cellVector.push_back(((Cell*)c->RDCell));
	}
	if (type != 2) {
		c->RUCell = new Cell();	((Cell*)c->RUCell)->setDepth(c->depth + 1);
		cellVector.push_back(((Cell*)c->RUCell));
	}
...
int main() {
	...
	fp = fopen("Test_07_25_uk_layer1_depth10_airpoil1_edit3.msh", "w");
