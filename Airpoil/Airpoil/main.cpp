...
bool manageEdge(vector<pdd> airpoile, double LeftDownX, double LeftDownY, double initRCellsize, int depth, int layer, double mincellsize) { // 재 : 끝부분 처리
	cout << "manageEdge 실행\n";
	//끝부분의 x값 비교
	bool flag = false;
	Node *NB, *NE;
	...
	if (NB->getX() != NE->getX()) {
		cout << "edge의 X값이 서로 일치하지 않습니다.\n";
		return flag;
	}

	//cutInnerCell을 위한 airpolie 추가 manageEdge가 cutInnerCell보다 앞에서 동작 이부분은 좀더 생각필
	inputLine.push_back(make_pair(*(itE), *(itB)));

	if (NB->getY() != NE->getY()) {
		cout << "manageEdge 해당\n";
		//initCell의 face에 걸린 경우
		if (NB->getX() - (NB->getX() / initRCellsize) == 0) {
			cout << "initCell의 face에 걸린 경우\n";
			//splitCell - 이상상황 없는지 fluent로 확인 필요
			initRectangle->splitCell(NE->getX(), NE->getY(), NB->getX(), NB->getY(), layer, depth, depth);
			//inout = -2 작업 // InnerCheck에서 실행
			flag = true;
			//wall추가
		...
		//tailcutManager의 대상 셀 사이에 수직사각형의 해당 셀이 없을 경우
		else if ((NB->getY() - NE->getY()) < mincellsize)
			cout << "tailcutManager만 실행\n";
		//수직 사각형
		else {
			cout << "수직 사각형\n";
			//mininode3 생성 - 전역변수 수정함
			//findmininode 실행, updown은 2
			findminnode(NE->getX(), NE->getY(), NB->getX(), NB->getY(), mincellsize, 2);
			//dotPoint 및 setupmininodeCell
			if (!mininode[2].empty()) {
...
void tailcutfour(Cell* c, pdd point) {//lface로들어온다는 가정하에진행
	c->LUCell = new Cell();	((Cell*)c->LUCell)->setDepth(c->depth + 1);
	c->LDCell = new Cell();	((Cell*)c->LDCell)->setDepth(c->depth + 1);
	c->RDCell = new Cell();	((Cell*)c->RDCell)->setDepth(c->depth + 1);
	c->RUCell = new Cell();	((Cell*)c->RUCell)->setDepth(c->depth + 1);
...
int main() {
	...
	int layer = 2, depth = 10; //진
	double mincellsize = 0; //진
	int Xcount = 16, Ycount = 16; //진_initcellsize설정
	double initRCellsize = 0.5; //진_initcellsize설정
	FILE * inputfp;
	int n, uppern, lowern, tail; // 욱 inputfile 읽어와서 첫줄 저장
	double x, y; // 좌표
	vector<pdd> airpoile;
	vector<pdd> airpoile2;
	bool vertical_horizontal = false; // true = vertical , false = horizontal 
	//욱 : 추가 : 이거 vertical_horizontal 계속 false인 채로 유지된다. 즉 어떤 값이 들어와도 horizontal로 인식
	bool y_increase;
	...
	} // 욱 : 문제점 : vertical_horizontal이 계속 false인 채로 유지된다. 중간에 판단을 위한 변화가 없다.
	//cutInnerCell();
	bool flag = false;
