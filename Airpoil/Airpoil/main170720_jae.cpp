	...
	if (wopoint == 0) {//LU
		...
		if (norf_s == 1) {
			if (!c->UFace->isParent()) { //child 가없다면 쪼갠다.
				...
				c->UFace->setCenterNode(UPCenter);
				UpFace = ((Cell*)c->UFace->sCell)->DFace->Child2; // 욱 : 170724 Child1 = > Child2
			}
			else {
		...
		if (norf_f == 1) {
			if (!c->DFace->isParent()) { //child 가없다면 쪼갠다.
				...
				c->DFace->setCenterNode(DownCenter);
				DownFace = ((Cell*)c->DFace->eCell)->UFace->Child2; // 욱 : 170724 Child1 = > Child2
			}
			else {
		...
		if (norf_s == 1) {
			if (!c->LFace->isParent()) { //child 가없다면 쪼갠다.
				...
				c->LFace->setCenterNode(LeftCenter);
				LeftFace = ((Cell*)c->LFace->sCell)->RFace->Child2; // 욱 : 170724 Child1 = > Child2
			}
			else {
				LeftCenter = c->LFace->centerNode;
...
int main() {
	...
	fp = fopen("Test_07_25_uk_layer1_depth07_hexa1_before.msh", "w");
